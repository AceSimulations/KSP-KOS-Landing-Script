...
UNTIL ALT:RADAR < 1000 {
    ...
    LOCAL TargetVector IS TARGET:POSITION.        //Target Vector
    ...
    //Flight Events
    //Entry Burn
    if ALT:RADAR < 21500 AND ALT:RADAR > 13000 {
      set throt to 1.
      lock steering to up.
      wait 0.
    }
    else if ALT:RADAR < 13000 AND ALT:RADAR > 10000 {
      set throt to .4.
      lock steering to vecTar.
      wait 0.
    }
    else if ALT:RADAR < 10000 AND ALT:RADAR > 8000 {
      set throt to .2.
      lock steering to vecTar.
      wait 0.
    }
    else if ALT:RADAR < 8000 AND SHIP:AIRSPEED > 500 {
      set throt to .2.
      lock steering to vecTar.
      set accelAdjust to 3.
      print "Velocity Adjust" at(0,32).
      wait 0.
    }
    else if ALT:RADAR < 8000 AND SHIP:AIRSPEED > 390 AND SHIP:AIRSPEED < 500 {
      set throt to .1.
      lock steering to vecTar.
      wait 0.
    }
...
CLEARSCREEN.
set radarOffset to 26.1.	 				// The value of alt:radar when landed (on gear)
