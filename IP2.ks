...
set driftAdjust to .0000. //droneship adjustment
...
if boostback = 0 {
  ...
  SET targetGeo TO LATLNG(DronePos:LAT,DronePos:LNG).
...
UNTIL ship:verticalspeed > -0.01 { 
    ...
    //droneship Entry
        ...
        if targetLoad = 1 {
          ...
          lock targetGeo TO LATLNG((DronePos:LAT + driftAdjust),DronePos:LNG).
        }
        set targetLoad to 0.
    ...
    //droneship landing translate
    else if trueRadar < (stopDist + 1000) AND ALT:RADAR < 5000 AND ALT:RADAR > 35 AND boostback = 0 {
        print "Landing Burn F2 [Nominal]" AT(0,30).
        Set line_of_sight to target:position - ship:position.
        Set gain to .2.
        set output to 1*up:vector * angleAxis(gain*vang(ship:velocity:surface,line_of_sight),vcrs(ship:facing:vector, ship:velocity:surface)).
        print output at(0,5).
        if landingStart = 1 {
            rcs on.
            sas off.
    ...
    else if boostback = 0 AND ALT:RADAR < 35 {
        set limit to 50.
        SHIP:PARTSDUBBED("fin")[0]:GETMODULE("ModuleControlSurface"):setfield("authority limiter", limit).
        SHIP:PARTSDUBBED("fin")[1]:GETMODULE("ModuleControlSurface"):setfield("authority limiter", limit).
        SHIP:PARTSDUBBED("fin")[2]:GETMODULE("ModuleControlSurface"):setfield("authority limiter", limit).
        ...
        else if SHIP:AIRSPEED < 400 AND ALT:RADAR < 10000 {
